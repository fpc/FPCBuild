...
help:
	...
	@$(ECHO) "   updateversion"
	@$(ECHO) "               Update version numbers (needs PACKAGE_VERSION set)"
	@$(ECHO) "   sourcezips"
	@$(ECHO) "               Create source zips/tar.gz"
	@$(ECHO)
	...
	@$(ECHO) "   innoandroid Build Win32-Android cross compilerinstaller"
...
.PHONY: all clean distclean build install installbase zipinstall singlezipinstall crosssinglezipinstall updateversion sourcezips
updateversion: install/updatever$(EXEEXT)
	install/updatever$(EXEEXT) $(PACKAGE_VERSION)
	cd fpcsrc && ../install/updmkver $(PACKAGE_VERSION)
install/updatever$(EXEEXT): install/updatever.pp
	fpc $< 
sourcezips: install/makesource
	install/makesource $(PACKAGE_VERSION)
all: build
build: $(BUILDSTAMP)
$(BUILDSTAMP):
	$(MAKE) -C fpcsrc build
...
renametoshort:
	...
	-$(MOVE) $(PKGPRE)utils-fpcmkcfg$(ZIPSUFFIX).zip fpck$(ZIPSUFFIXSHORT).zip
	...
	-$(MOVE) $(PKGPRE)utils-lexyacc$(ZIPSUFFIX).zip lexy$(ZIPSUFFIXSHORT).zip
